...
if [ ! -f "$FW_PATH" ]; then
    jq -n --arg msg "Firmware file does not exist" \
          '{status: "failed", message: $msg, progress: 0}'
    exit 1
fi
...
sudo systemctl stop mavlink-router 2>/dev/null
...
python3 -u pi/scripts/px_uploader.py --json-progress --port $SERIALDEVICE $FW_PATH 2>&1 | while IFS= read -r line
do
    echo "$line" | jq -c 'select(type == "object")' 2>/dev/null || :
done

sudo systemctl start mavlink-router logloader.service 2>/dev/null
